...
async function scrapeTableData() {
    try {
        ...
        await page.goto('https://sportsbook.draftkings.com/leagues/soccer/euro-2024-qualifying', {
        ...
        await page.evaluate(async () => {
            ...
            games.forEach(async (game) => {
                console.log(game.querySelector('.sportsbook-event-accordion__title'))
                const button = game.querySelector('.sportsbook-event-accordion__title')
                button.click()
                await new Promise(function(resolve) {setTimeout(resolve, 5000)});
                console.log('+++++++++++++++')
                const moneylineDiv = document.querySelector('[aria-label="Event Accordion for Draw No Bet (Regular Time)"]')
